...
{
    internal class DataGridStateManagerForFuturesPositions : DataGridStateManagerForPositions
    {
        private string selectedHugoSymbol;
        private string selectedMerrillSymbol;

        public DataGridStateManagerForFuturesPositions(DataGridView dataGridView,
        ...
        public void SetSelectedSymbols(out bool symbolsSet, out bool sameRow)
        {
            ...
            {
                if ("FuturesSymbol" == dataGridView.Columns[cell.ColumnIndex].DataPropertyName)
                {
                    nCellCount++;
